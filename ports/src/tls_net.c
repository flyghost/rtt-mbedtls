...
/**
 * @brief 根据给的 host:port和protocal，初始化一个TCP连接
 * 
 * @param ctx NET 网络配置结构体对象
 * @param host 指定的待连接主机名
 * @param port 指定的主机端口号
 * @param proto 指定的协议类型，MBEDTLS_NET_PROTO_TCP 或者 MBEDTLS_NET_PROTO_UDP
 * 
 * @return 0                                连接成功
 * @return MBEDTLS_ERR_NET_SOCKET_FAILED    socket 创建失败
 * @return MBEDTLS_ERR_NET_UNKNOWN_HOST     未知的主机名，DNS 解析失败
 * @return MBEDTLS_ERR_NET_CONNECT_FAILED   网络连接失败
 */
